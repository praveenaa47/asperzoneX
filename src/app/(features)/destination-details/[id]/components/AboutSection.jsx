export default function About({highlights }) {
  
  const features = highlights?.length
    ? highlights.map((item, index) => ({
        id: item._id || index,
        title: item.title,
        image: item.icon, // Use API icon instead of static gif
        alt: item.title,
      }))
    : [
        {
          id: 1,
          title: "Best Price Guarantee",
          image: "/bestprice.gif",
          alt: "Trusted Expertise",
        },
        {
          id: 2,
          title: "Trusted hotel partners",
          image: "/system.gif",
          alt: "Hassle-Free Process",
        },
        {
          id: 3,
          title: "24/7 customer support",
          image: "/support.gif",
          alt: "Premium Support",
        },
        {
          id: 4,
          title: "Customized travel plans",
          image: "/travelexp.gif",
          alt: "Transparent Deals",
        },
      ];
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8 sm:py-9 md:py-8 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center text-black mb-8 sm:mb-12 md:mb-16">
        Key Highlights
        </h2>
        
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
          {features.map((feature) => (
            <div 
              key={feature.id} 
              className="flex flex-col items-center text-center p-4 sm:p-6 md:p-8  transition-shadow duration-300 "
            >
              <div className="mb-3 sm:mb-4 md:mb-6">
                <img 
                  src={feature.image} 
                  alt={feature.alt}
                  className="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 object-contain"
                />
              </div>
              <h3 className="text-xs sm:text-base md:text-lg font-semibold text-gray-800 leading-tight">
                {feature.title}
              </h3>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
}