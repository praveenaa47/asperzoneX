...
export default function FeaturedProperties() {
  ...
  // Fix: Access the nested array properly
  const featuredProperties = data?.data?.filter((item) => item.isFeatured === true) || [];
  ...
  const handlePropertyClick = (property) => {
    router.push(`/realestate-details/${property.id}`);
  };
            ...
            <div className="relative h-44 sm:h-48">
              ...
              <button
                ...
                className={`absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                  favorites.includes(property.id || property._id)
              ...
              >
                <Heart
                  className={`w-4 h-4 ${
                    favorites.includes(property.id || property._id) ? "fill-current" : ""
                  }`}
                />
              </button>
            </div>
